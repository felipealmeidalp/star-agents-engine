...
Helena's session send (count, target sessionId, message text, ORDER). The path runs
...
LEAD_PHONE = "+5511999998888"  # details.from — kept only in the customer record
...
def patch_send_text(sends: list[tuple[str, str]]) -> Any:
    """Patch HelenaClient.send_text to record each call as (session_id, message).

    Helena routes the reply by the conversation's sessionId
    (POST /v1/session/{id}/message) — the only send contract that works on
    non-WhatsApp channels (see the client docstring / API docs).
    """

    async def _stub(
        client_self: Any, session_id: str, message: str, apikey: str
    ) -> dict[str, str]:
...
async def case_a_text_dispatches() -> None:
    ...
    assert calls[0][0] == SESSION_ID, calls[0][0]
    assert any(sent == reply for _sid, sent in calls), calls
...
async def case_e_n_messages_ordered() -> None:
    ...
    assert resp.status_code == 200, resp.status_code
    sent_texts = [sent for _sid, sent in calls]
    assert sent_texts == replies, f"expected {replies} in order, got {sent_texts}"
    assert all(sid == SESSION_ID for sid, _ in calls), calls
...
async def case_k_transcription_failure_alerts_and_notifies() -> None:
    ...
    assert sends[0][0] == SESSION_ID, sends[0][0]
