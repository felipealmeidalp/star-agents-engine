...
Helena's send/text (count, target sessionId, message text, ORDER). The path runs
...
def patch_send_text(sends: list[tuple[str, str]]) -> Any:
    ...
    async def _stub(
        ...
        return {"id": "queued", "sessionId": session_id, "status": "QUEUED"}
...
async def case_a_text_dispatches() -> None:
    ...
    print("  (a) text MESSAGE_RECEIVED → dispatched to sessionId: OK")
